...
def draw_smile(color_element=sd.COLOR_YELLOW):
    ...
    sd.ellipse(right_top=right_top, left_bottom=left_bottom, color=color_element, width=1)
    ...
    sd.lines(point_list=point_list, color=color_element)

    center_position1 = sd.get_point(x + 50, y + 125)
    radius = 7
    sd.circle(center_position=center_position1, radius=radius, color=color_element, width=1)

    center_position2 = sd.get_point(x + 100, y + 125)
    sd.circle(center_position=center_position2, radius=radius, color=color_element, width=1)
